...
void loadPersistentSettings() {
    ...
    // Si falta usuario o pass, marcamos que necesita configuración
    globalState.needsConfig = (globalState.qrzUser.empty() || globalState.qrzPass.empty());
    
    // Si ya hay datos, intentamos activar el motor de inicio
    if (!globalState.needsConfig) globalState.isProcessing = true;
...
void web_init() {
    ...
    std::thread serverThread([]() {
        ...
        svr.Get("/api/settings", [](const httplib::Request&, httplib::Response& res) {
            ...
            {
                std::lock_guard<std::mutex> lock(globalState.mtx);
                j["user"] = globalState.qrzUser;
                j["pass"] = globalState.qrzPass;
                j["myCall"] = globalState.myCallsign;
                j["band"] = globalState.currentBand;
        ...
        svr.Get("/api/report", [](const httplib::Request&, httplib::Response& res) {
            ...
            
            {
                ...
                for (const auto& c : globalState.validatedContacts) {
                    std::string call = c["call"].get<std::string>();
                    std::string name = c["name"].get<std::string>();
                    
                    adif << "<CALL:" << call.length() << ">" << call << " ";
                    adif << "<NAME:" << name.length() << ">" << name << " ";
                    adif << "<BAND:" << globalState.currentBand.length() << ">" << globalState.currentBand << " ";
                    adif << "<MODE:2>FM <EOR>\n";
                }
            }

            res.set_content(adif.str(), "text/plain");
            res.set_header("Content-Disposition", "attachment; filename=logbook.adi");
        });
        ...
        svr.Post("/api/settings", [&](const httplib::Request& req, httplib::Response& res) {
            try {
                ...
                std::string newUser = j.at("user").get<std::string>();
                std::string newPass = j.at("pass").get<std::string>();
                std::string newCall = j.at("myCall").get<std::string>();
                std::string newBand = j.at("band").get<std::string>();

                // 1. Intentar Login en QRZ con las nuevas credenciales
                qrz.init(newUser, newPass); // Re-configuramos el cliente QRZ
                
                if (qrz.login()) {
                    // 2. Si el login es exitoso, actualizamos el estado global
                    {
                        std::lock_guard<std::mutex> lock(globalState.mtx);
                        globalState.qrzUser = newUser;
                        globalState.qrzPass = newPass;
                        globalState.myCallsign = newCall;
                        globalState.currentBand = newBand;
                        
                        globalState.needsConfig = false;  // ¡Ya podemos trabajar!
                        globalState.isProcessing = true; // Arrancamos el motor automáticamente
                    }

                    // 3. Persistimos en el archivo .env
                    updateEnvFile(newUser, newPass, newCall);

                    res.status = 200;
                    res.set_content("{\"status\":\"ok\"}", "application/json");
                    std::cout << "[CONFIG] Ajustes actualizados y QRZ conectado." << std::endl;
                } else {
                    res.status = 401; // No autorizado
                    res.set_content("{\"error\":\"Login de QRZ fallido\"}", "application/json");
                }
            } catch (const std::exception& e) {
                res.status = 400;
